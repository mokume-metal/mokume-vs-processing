...
| | |
| --- | --- |
| [`run.py`](run.py) | 負荷の段ごとに両側を起こし、記録を集めて表にする (端末用の表と、貼り付け用の `summary.md`)。全段を `--repeat` 回 (既定 3) 掃き、回ごとに先に回す側を入れ替える |
| [`stats.py`](stats.py) | 間隔の列の要約 (fps・分位) と、繰り返した回のまとめ (中央値・fps の最小〜最大) |
